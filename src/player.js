class Player {
    ...
    static playing(frame) {
        ...
        if (this.keyStatus.right || this.keyStatus.left) {
            ...
            const mt = y + this.puyoStatus.dy;
            // その方向にブロックがないことを確認する
            // まずは自分の左右を確認
            let canMove = true;
            if (y < 0 || x + cx < 0 || x + cx >= Config.stageCols || Stage.borad[y][x + cx]) {
                if (y >= 0) {
                    canMove = false;
                }
            }
            if (my < 0 || mx + cx < 0 || x + cx >= Config.stageCols || Stage.board[my + 1][mx + cx]) {
                if (my + 1 >= 0) {
                    canMove = false;
                }
            }
            ...
            if (this.groundFrame === 0) {
                if (y + 1 < 0 || x + cx < 0 || x + cx >= Config.stageCols || Stage.board[y + 1][mx + cx]) {
                    if (y + 1 >= 0) {
                        canMove = false;
                    }
                }
            ...
            if (rotation === 0) {
                ...
                // 上から左に回すとき、左にブロックがあれば右に移動する必要があるのでまず確認する
                ...
                // 右にずれる必要があるとき、右にもぶろっがあれば回転できないので確認する
                if (cx === 1) {
            ...
            if (canRotate) {
                ...
                // 回すことが出来るので、開店後の情報をセットして回転状態にする
                this.actionStartFrame = frame;
    ...
    static rotating(frame) {
        ...
        this.setPuyoPosition();
        if (ratio === 1) {
            this.puyoStatus.rotation = (this.this.rotateFromRotation + 90) % 360;
            return false;
        }
        return true;
    }
    ...
    static fix() {
        ...
        if (y >= 0) {
            // 画面外のぷよは消してしまう
            Stage.setPuyo(x, y, this.movablePuyo);
            Stage.puyoCount++;
        }
        if (y + dy >= 0) {
            // 操作用に作成したぷよ画像を消す
